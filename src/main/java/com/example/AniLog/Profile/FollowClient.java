...
public class FollowClient {
    ...
    @PostMapping("/{userId}/accept-request/{targetUserId}")
    public ResponseEntity<String> acceptFollowRequest(@PathVariable Long userId, @PathVariable Long targetUserId) {
        try {
            followService.acceptFollowRequest(userId, targetUserId);
    ...
    @DeleteMapping("/{userId}/decline-request/{targetUserId}")
    public ResponseEntity<String> declineFollowRequest(@PathVariable Long userId, @PathVariable Long targetUserId) {
        try {
            followService.denyFollowRequest(userId, targetUserId);
            return ResponseEntity.ok("Follow request declined successfully");
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
